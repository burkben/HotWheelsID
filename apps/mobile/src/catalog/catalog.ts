 ...
 * with the app. Artwork is kept out of this snapshot and lives in `artwork.ts` /
 * `artwork.json`, so the pinned catalog hash stays stable when images change
 * (ADR-0013).
 *
...
export interface CatalogCar {
  ...
  /** Always null — artwork is keyed by catalog id in `artwork.json` instead. */
  readonly image: null;
