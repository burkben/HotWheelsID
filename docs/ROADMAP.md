...
| 5 — Delight & depth | 🟡 In progress | Achievements shipped; richer car identity, race-night lineup with tournament mode, race sound cues, **TV/host mode on a real external display**, and **iPad two-pane layouts** shipped; simultaneous multi-lane remains. |
...
- ✅ Sound design and "TV/host mode." **Race sound cues shipped** — countdown ticks/go,
  ...
  rebuild. **TV/host mode now ships as a genuine second display**, not mirroring: a local Expo
  module (`modules/external-display`) mounts a second React surface onto the iOS
  `.windowExternalDisplayNonInteractive` scene, so the TV runs its own big-screen `TvStage` while
  the device keeps the controls. Both surfaces share one JS runtime, so the Zustand stores drive
  the TV with no state bridging. See [ADR-0015](adr/0015-external-display-tv-mode.md).
- ✅ iPad-shaped layouts. The app no longer renders as a stretched phone: iPad rotates freely
  (iPhone stays portrait), and a pure `resolveLayout(w, h)` breakpoint module drives two-pane
  Speed and Race screens (gauge / live race on the left, passes / laps / leaderboard on the right)
  plus multi-column Garage and History grids. Driven by the *window*, so Split View and Stage
  Manager behave.
